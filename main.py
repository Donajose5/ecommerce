...
def productDescription():
	...
	if request.method=='POST':
		...
		with sqlite3.connect('database.db') as conn:
			...
			userId = cur.fetchone()[0]
			cur.execute('INSERT into reviews VALUES(?,?,?,?)', (userId,productId,rating,review))
		conn.close()
	with sqlite3.connect('database.db') as conn:
		cur = conn.cursor()
		...
		cur.execute('SELECT userId FROM users WHERE email = ?', (email, ))
		userId = cur.fetchone()[0]
		cur.execute('SELECT rating, firstName, description FROM reviews, users WHERE reviews.userId = users.userId and productId = ?', (productId, ))
	...
	if count!=0:
		total_rating/=count
	return render_template("productDescription.html", data=productData, loggedIn = loggedIn, firstName = firstName, noOfItems = noOfItems, reviewData=reviewData, rating=total_rating)
